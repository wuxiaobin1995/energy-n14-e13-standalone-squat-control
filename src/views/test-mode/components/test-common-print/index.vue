...
-->
<template>
  <div class="test-common-print" v-loading.fullscreen.lock="fullscreenLoading">
    <div class="top">
      <div class="left">
        <div class="title">{{ testData.testType }}报告</div>
        <div class="hospital">( {{ testData.hospital }} )</div>
        <div class="info">
          <div class="item">姓名：{{ testData.userName }}</div>
          <div class="item">性别：{{ testData.sex }}</div>
          <div class="item">患侧：{{ testData.affectedSide }}</div>
          <div class="item">测试时间：{{ testData.pdfTime }}</div>
        </div>
      </div>
        ...
        <div class="left-top">
          ...
          <div class="num-show">
            <div class="left-weight">
              ...
              <div class="num-data">
                <div class="data">{{ testData.leftAverageWeight }}</div>
                <div>kg</div>
              </div>
            </div>
            <div class="right-weight">
              ...
              <div class="num-data">
                <div class="data">{{ testData.rightAverageWeight }}</div>
                <div>kg</div>
              </div>
            </div>
          </div>
        ...
        <div class="left-bottom">
          ...
          <div class="num-show">
            <div class="left-percent">
              <div class="num-text">左</div>
              <div class="num-data">
                <div class="data">{{ testData.leftAverageWeightPercent }}</div>
                <div>%</div>
              </div>
            </div>
            <div class="right-percent">
              ...
              <div class="num-data">
                <div class="data">
                  {{ testData.rightAverageWeightPercent }}
                </div>
                <div>%</div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    getTestData() {
      ...
      db.test_data
        ...
        .then(() => {
          ...
          for (
            let i = 0;
            i < this.testData.rightWeightPercentArray.length;
            i++
          ) {
            if (
              47.5 <= this.testData.rightWeightPercentArray[i] &&
              this.testData.rightWeightPercentArray[i] <= 52.5
            ) {
              this.greenArray.push(this.testData.rightWeightPercentArray[i])
            } else if (
              (40 <= this.testData.rightWeightPercentArray[i] &&
                this.testData.rightWeightPercentArray[i] < 47.5) ||
              (52.5 < this.testData.rightWeightPercentArray[i] &&
                this.testData.rightWeightPercentArray[i] <= 60)
            ) {
              this.yellowArray.push(this.testData.rightWeightPercentArray[i])
            } else {
              this.redArray.push(this.testData.rightWeightPercentArray[i])
            }
          }
          this.greenPercent = parseFloat(
            (
              (this.greenArray.length /
                this.testData.rightWeightPercentArray.length) *
              100
            ).toFixed(1)
          )
    ...
    initChart() {
      ...
      const rl = []
      const rr = []
      for (let i = 0; i < this.testData.rightWeightPercentArray.length; i++) {
        gl.push(47.5)
        gr.push(52.5)
        yl.push(40)
        yr.push(60)
      ...
      this.option = {
        ...
        series: [
          ...
          },
          {
            ...
            smooth: true,
            symbol: 'none',
            data: this.testData.rightWeightPercentArray,
            lineStyle: {
              color: 'rgba(0, 0, 0, 1)'
            }
          }
